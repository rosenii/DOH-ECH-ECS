    ...
    try {
        ...
        if (qName === "cf.ech" || qName === "fb.ech") {
            if (qType === 65) {
                ...
                if (qName === "cf.ech") {
                    const echRdata = await fetchCleanEchRdataWithHints(config, qName, ctx, clientIP);
                    return dnsResponse(createMultiAnsResponse(id, qName, 65, echRdata ? [echRdata] : [], echRdata ? randomTtl : 60));
                }
                if (qName === "fb.ech") {
                    const echRdata = await packMetaEchWithHints(qName, config, clientIP);
        ...
        if (isStaticCF || isStaticMeta) {
            if (qType === 28) {
                ...
                if (config.ip6) ipStrings = parseIpList(config.ip6);
                    ...
                    const ips = await resolveMultiDomainToIps(config.cfDomain, 28, clientIP);
            ...
            if (qType === 1) {
                ...
                if (isStaticCF) {
                    if (config.ip4) ipStrings = parseIpList(config.ip4);
                        ...
                        const ips = await resolveMultiDomainToIps(config.cfDomain, 1, clientIP);
    ...
    if (isStatic) {
        if (type === 'AAAA') {
            ...
            if (config.ip6) ipList = parseIpList(config.ip6);
                ...
                const resolved = await resolveMultiDomainToIps(config.cfDomain, 28, clientIP);
...
async function fetchCleanEchRdataWithHints(config, domain, ctx, clientIP) {
    ...
    const realEch = await fetchRealEch(config.echDomain || 'cloudflare-ech.com', clientIP);
    ...
    <div class="container">
        ...
        <div id="cfParams" class="advanced-section">
            <div class="param-group active" id="cfGroup">
                ...
                <label>全局跟随优选 <span class="badge badge-cf">best</span></label>
                <select id="best">
